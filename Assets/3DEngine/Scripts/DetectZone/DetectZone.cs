...
public class DetectZone
{
    ...
    private CoroutineHandle detectRoutine = new CoroutineHandle();
    private CoroutineHandle triggerRoutine = new CoroutineHandle();
    ...
    public void AddEnterTrigger(GameObject _sender, System.Action<Collider> _callback)
    {
        enterCallBacks.Add(_callback);
        DoTriggerDetection(_sender);  
    }

    public void RemoveEnterTrigger(GameObject _sender, System.Action<Collider> _callback)
    {
        enterCallBacks.Remove(_callback);
        CheckCallBacks(_sender);
    }

    public void AddExitTrigger(GameObject _sender, System.Action<Collider> _callback)
    {
        exitCallBacks.Add(_callback);
        DoTriggerDetection(_sender);
    }

    public void RemoveExitTrigger(GameObject _sender, System.Action<Collider> _callback)
    {
        exitCallBacks.Remove(_callback);
        CheckCallBacks(_sender);
    }

    public void AddStayTrigger(GameObject _sender, System.Action<Collider> _callback)
    {
        stayCallBacks.Add(_callback);
        DoTriggerDetection(_sender);
    }

    public void RemoveStayTrigger(GameObject _sender, System.Action<Collider> _callback)
    {
        stayCallBacks.Remove(_callback);
        CheckCallBacks(_sender);
    }

    public void ClearEnterTriggers(GameObject _sender)
    {
        enterCallBacks.Clear();
        CheckCallBacks(_sender);
    }

    public void ClearExitTriggers(GameObject _sender)
    {
        exitCallBacks.Clear();
        CheckCallBacks(_sender);
    }

    public void ClearStayTriggers(GameObject _sender)
    {
        stayCallBacks.Clear();
        CheckCallBacks(_sender);
    }

    public void ClearAllTriggers(GameObject _sender)
    {
        stayCallBacks.Clear();
        enterCallBacks.Clear();
        exitCallBacks.Clear();
    ...
    void DoTriggerDetection(GameObject _sender)
    {
        if (detectRoutine == new CoroutineHandle())
            detectRoutine = Timing.RunCoroutine(StartColliderDetection(_sender.transform).CancelWith(_sender), Segment.FixedUpdate);
        if (triggerRoutine == new CoroutineHandle())
            triggerRoutine = Timing.RunCoroutine(StartTriggerDetection().CancelWith(_sender), Segment.FixedUpdate);
    }

    void CheckCallBacks(GameObject _sender)
    {
        if (stayCallBacks.Count < 1 && enterCallBacks.Count < 1 && exitCallBacks.Count < 1)
        {
            if (detectRoutine != new CoroutineHandle())
                Timing.KillCoroutines(detectRoutine);
            if (triggerRoutine != new CoroutineHandle())
                Timing.KillCoroutines(triggerRoutine);

            detectRoutine = new CoroutineHandle();
            triggerRoutine = new CoroutineHandle();
        }
    }

    IEnumerator<float> StartColliderDetection(Transform _sender)
    {
    ...
    IEnumerator<float> StartTriggerDetection()
    {
        while (Application.isPlaying)
        {
            CheckEnterTrigger();
    ...
    void CheckExitTrigger()
    {
        ...
        for (int i = 0; i < enteredList.Count; i++)
        {
            ...
            else//yes
            {
                ...
                for (int ind = 0; ind < stayCallBacks.Count; ind++)
                {
                    stayCallBacks[ind].Invoke(enteredList[i]);
                }
            }
        }
    }
