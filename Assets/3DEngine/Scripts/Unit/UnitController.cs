...
public class UnitController : MonoBehaviour
{
    ...
    public enum MovementStateType { Idle, Patrol, Wander, LookAt, Chase, Flee }
    ...
    //movement
    [SerializeField] protected MovementStateType defaultMovement = MovementStateType.Idle;
    ...
    [SerializeField] protected float arrivalTime = 0.1f;
    ...
    protected CoroutineHandle disableNavCoroutine;
    ...
    {
        //movement type
        curMovementState = defaultMovement;

        if (curMovementState == MovementStateType.Patrol)
        {
        ...
        else if (curMovementState == MovementStateType.Wander)
        {
            //wander stuff
    ...
    protected virtual void CheckFleeTarget()
    {
        ...
        if (curMovementState != MovementStateType.Flee)
            return;
    ...
    protected virtual void CheckChaseTarget()
    {
        ...
        if (curMovementState != MovementStateType.Chase)
            return;
    ...
    protected virtual void CheckLookAtTarget()
    {
        ...
        if (curMovementState != MovementStateType.LookAt)
            return;
    ...
    {
        curChaseTarget = RefreshTargetList(chaseTargets, chasePriorities, MovementStateType.Chase);
    }

    void RefreshLookAtTarget()
    {
        curLookAtTarget = RefreshTargetList(lookAtTargets, lookAtPriorities, MovementStateType.LookAt);
        if (curLookAtTarget)
        {
            var unit = curLookAtTarget.GetComponent<Unit>();
            if (unit)
            {
                if (unit.AttackTarget)
                    curLookAtTarget = unit.AttackTarget;
            }
        }
    ...
    {
        curFleeTarget = RefreshTargetList(fleeTargets, fleePriorities, MovementStateType.Flee);
    }

    Transform RefreshTargetList(List<Transform> _targets, EntityPriority[] _priorities, MovementStateType _moveType)
    {
        if (_targets.Count < 1)
        {
            if (curMovementState == _moveType)
                DoDefaultMovement();
        ...
        if (tar == null && curMovementState == _moveType && doDefaultMovementIfNoTargets)
            DoDefaultMovement();
    ...
    protected virtual void CheckMovement()
    {
        ...
        if (curMovementState == MovementStateType.Patrol)
            Patrol();
        else if (curMovementState == MovementStateType.LookAt)
            LookAt();
        else if (curMovementState == MovementStateType.Wander)
            Wander();
        else if (curMovementState == MovementStateType.Chase)
            Chase();
        else if (curMovementState == MovementStateType.Flee)
            Flee();
        else if (curMovementState == MovementStateType.Idle)
            Idle();
    }
    ...
    protected virtual void CheckSideHits()
    {
        ...
        if (sideCols.Length > 0)
        {
            ...
            foreach (var col in sideCols)
            {
                ...
                slowCols = slowMask == (slowMask | (1 << layer));
    ...
    public virtual void DoDefaultMovement(Transform _target = null)
    {
        ...
        if (defaultMovement == MovementStateType.Wander)
        {
    ...
    public virtual void ChaseTarget(Transform _target)
    {
        ...
        if (_target == curChaseTarget && curMovementState == MovementStateType.Chase)
            return;
        
        AddChaseTarget(_target);
        chaseTimer = chaseIntervalCheckTime;
        SetCurMovementState(MovementStateType.Chase);
    }
    ...
    public virtual void FleeTarget(Transform _target)
    {
        ...
        if (_target == curFleeTarget && curMovementState == MovementStateType.Flee)
            return;
        AddFleeTarget(_target);
        fleeTimer = fleeIntervalCheckTime;
        curMovementState = MovementStateType.Flee;
    }
    ...
    public virtual void LookAtTarget(Transform _target)
    {
        ...
        SetCurMovementState(MovementStateType.LookAt);
    }
    ...
    public void DisableNavAgent(float _time, bool _rbKinematic, bool _enableIfGrounded)
    {
        if (!agent)
            return;

        if (disableNavCoroutine != null)
            Timing.KillCoroutines(disableNavCoroutine);
        disableNavCoroutine = Timing.RunCoroutine(StartDisableNavAgent(_time,_rbKinematic, _enableIfGrounded));
    }

    IEnumerator<float> StartDisableNavAgent(float _time, bool _rbKinematic, bool _enableIfGrounded)
    { 
