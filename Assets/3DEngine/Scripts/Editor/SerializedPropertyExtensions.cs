...
public static class SerializedPropertyExtensions
{
    ...
    public static bool IsInArray(this SerializedProperty _property)
    {
    ...
    public static SerializedProperty GetArrayOfElement(this SerializedProperty _element, out int _index)
    {
        var path = _element.propertyPath;
        _index = 0;
        if (path.Contains("["))
        {
            var pathfixed = path.Replace(".Array.data[", "[");
            ...
            var arrayName = split[split.Length - 1];
            _index = Convert.ToInt32(new string(arrayName.Where(c => char.IsDigit(c)).ToArray()));
            //delete brackets so we find just the name array field
            arrayName = arrayName.Substring(0, arrayName.IndexOf("["));
            return _element.serializedObject.FindProperty(arrayName);
        }
        return null;
    }

    public static int LastIndex(this SerializedProperty _arrayProperty)
    {
        if (_arrayProperty.arraySize == 0)
            return 0;
        else
            return _arrayProperty.arraySize - 1;
    }
}