...
public class PlayerController : UnitController
{
    ...
    void Crouching()
    {
        ...
        {
            crouching = false;
            ResetColliderHeight(crouchTime, 0, grounded || (!grounded && velocityDirection.y < 0));
            return;
        }
        ...
        LerpColliderHeight
            ...
            startColHeight / 2, crouchTime, 0, grounded || (!grounded && velocityDirection.y < 0));
    ...
    IEnumerator<float> StartLerpColliderHeight(Vector3 _center, float _height, float _time, float _delay, bool _grounded, System.Action _onFinishedCallback = null)
    {
        ...
        while (perc != 1)
        {
            ...
            if (!_grounded)//in air "crouch jump"
            {
    ...
    void InAir()
    {
        if (inputLocalDirection != Vector3.zero && sideCols.Length == 0)
        {
            if (controlFading)
                return;
    ...
    protected virtual void Move()
    {
        ...
        //prevent player jamming into walls
        if (!grounded && sideCols.Length > 0 || jump && sideCols.Length > 0)
        {
