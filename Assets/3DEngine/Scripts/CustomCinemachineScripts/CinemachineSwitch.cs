...
public class CinemachineSwitch
{
    ...
    IEnumerator<float> StartFindTargets(int _ind)
    {
        ...
        if (camData.setFollowTarget)
        {
            ...
            {
                camData.Follow = camData.followTarget.GetSceneObject(master.gameObject, master.gameObject);
        ...
        if (camData.setLookAtTarget)
        {
            while (!camData.LookAt)
            {
                if (camData.followAndLookatIdentical)
                    camData.LookAt = camData.followTarget.GetSceneObject(master.gameObject, master.gameObject);
                else
                    camData.LookAt = camData.lookAtTarget.GetSceneObject(master.gameObject, master.gameObject);
                yield return Timing.WaitForOneFrame;
            }
        }
